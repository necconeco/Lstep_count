...
export interface FullHistoryRecord {
  ...
  visitLabel: VisitLabel; // 来店ラベル（初回, 2回目, 3回目以降）
  // 重複判定用フォールバックキー
 ...
 * masterTypes.ts の FlattenedRecord と統合
 */
export interface FlattenedRecord {
  ...
  visitLabel: VisitLabel | '-'; // 実施の場合のみ有効
}
...
/**
 * マスターマージ処理用CSVレコード
 * masterMerge.ts で使用（旧 masterTypes.ts の CsvInputRecord）
 */
export interface MasterCsvInputRecord {
  reservationId: string;
  friendId: string;
  date: Date; // 予約日
  status: ReservationStatus;
  ...
  staff: string | null;
  detailStatus: string | null;
  applicationDate: string; // 申込日時（文字列形式）
}

// ============================================================================
// 旧 types/index.ts との互換性用エイリアス
// ============================================================================

/**
 * 来店タイプ（旧 VisitType との互換性）
 * @deprecated VisitLabel を使用してください
 */
export type VisitType = VisitLabel;
