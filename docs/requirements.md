# Lステップ集計ツール - 要件定義書 v2.2

**最終更新**: 2026-01-19
**バージョン**: 2.2（認証・クラウド同期対応版）
...
- **認証済みユーザー**: パスワード認証後、すべての機能にアクセス可能

---

### 2.3 認証・認可要件

- 認証方式: **パスワード認証**（SHA-256ハッシュ照合）
- パスワード: `Lstep2025!`
- セッション管理: sessionStorage（ブラウザタブ単位）
- セキュリティレベル: **データはブラウザ内で処理** + **Supabaseクラウド同期**
- 管理機能: 不要
...
データベース:
  ローカル: IndexedDB（ブラウザ内蔵）
  クラウド: Supabase（PostgreSQL）
  用途: マスターデータ、過去の集計結果
  同期: 自動同期（CSVアップロード時にクラウドバックアップ、起動時にクラウドから復元）
...
| Supabase | クラウドDB・マスターデータ同期 | https://supabase.com | 無料プラン、500MB/月まで無料 |

### Supabase設定

**プロジェクト情報**:
- プロジェクト名: necconeco's Project
- URL: `https://wleixwilfcfwlrzkymkb.supabase.co`
...
CREATE TABLE user_visit_history (
  ...
  last_implementation_date TIMESTAMPTZ,
...
1. **CSVアップロード時**: IndexedDBに保存後、自動でSupabaseにバックアップ
2. **アプリ起動時**: ローカル（IndexedDB）が空の場合、Supabaseから自動復元
3. **エラー時**: クラウド同期失敗してもローカル動作は継続（非ブロッキング）
...
- **v2.2 (2026-01-19)**: パスワード認証追加、Supabaseクラウド同期機能追加
